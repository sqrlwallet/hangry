...
interface StrainCalculator {
     ...
     * (minutes in the user's own heart-rate zones, sleep excluded); falls back to a
     * workout-only estimate when there isn't enough continuous heart-rate data for the day.
     */
    fun calculateDayStrain(
        heartRateSamples: List<HeartRateSampleEntity>,
        workoutsToday: List<ExerciseSessionEntity>,
        zones: HeartRateZones = HeartRateZones.DEFAULT,
        sleepSessions: List<SleepSessionEntity> = emptyList()
    ): StrainResult
